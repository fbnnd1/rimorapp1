...
function gera_html_lugar (obj_lugar) {
    ...
    str_dados_exb = str_dados_exb.replace("@@dica2@@", obj_lugar.dicaExtra);
...
function carrega_info_mapa() {
    ...
    var arr_centro = [obj_mapa.pontos[0].latitude, obj_mapa.pontos[0].longitude];

    for (i in obj_mapa.pontos) {
        //arr_coords = [obj_mapa.pontos[i].longitude, obj_mapa.pontos[i].latitude];
        arr_coords = [obj_mapa.pontos[i].latitude, obj_mapa.pontos[i].longitude];
    ...
    desenha_mapa(arr_objs_features, arr_centro);
